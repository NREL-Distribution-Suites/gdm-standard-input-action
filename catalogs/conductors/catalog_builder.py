...
def build_conductor_models() -> list[Component]:
    ...
    for i, conductor_info in data.iterrows():
        cond_name = (
            conductor_info["Material"]
            + "_"
            + str(conductor_info["Size"])
            + "_"
            + str(conductor_info["Stranding"])
        )
    ...
    for i, conductor_info in conductor_data.iterrows():
        ...
        cond_name = (
            conductor_info["Code"] if str(conductor_info["Code"]) != "nan" else n
        )
    ...
    for i, cable_info in cable_data.iterrows():
        ...
        if str(n_strands_neutral) != "nan":
            ...
            if not conductor.empty and not neutral.empty:
                ...
                insulation_diameter = (
                    cond_diam + insulation_thickness_inches * 2
                ) * 1.001
                ...
                if neutral["Steel Core (Inches)"] > 0:
                    ...
                    try:
                        cable = ConcentricCableEquipment(
                            ...
                            cable_diameter=PositiveDistance(cable_diameter, "inch"),
                            phase_ac_resistance=PositiveResistancePULength(
                                conductor["Resistance-  ac 75 °C (Ω/kft)"],
                                "ohm/kilofeet",
                            ),
                            conductor_diameter=PositiveDistance(
                                conductor["Conductor Diameter (in)"], "inch"
                            ),
                            conductor_gmr=PositiveDistance(
                                conductor["GMR -ft"], "inch"
                            ),
                            strand_ac_resistance=PositiveResistancePULength(
                                neutral["Resistance-  ac 75 °C (Ω/kft)"], "ohm/kilofeet"
                            ),
    ...
    for i, cable_info_2 in cable_data_2.iterrows():
        ...
        insulation_thickness_inches = (
            cable_info_2["Insulation Thickness (mils) "] / 1000.0
        )
        ...
        cond_name = (
            conductor + "_" + nphases + "_" + kv + "_" + conc_neutral + "_" + material
        )
        ...
        cable = ConcentricCableEquipment(
            ...
            strand_diameter=PositiveDistance(
                neutral["Conductor Diameter (in)"] / 10.0, "inch"
            ),
            strand_gmr=PositiveDistance(neutral["GMR -ft"], "inch"),
