...
from gdm import DistributionTransformerEquipment, WindingEquipment, DistributionSystem, ConnectionType, VoltageTypes, SequencePair
...
class Model:
    ...
    def build_models_1(self):
        ...
        reg_data_1 = pd.read_csv(self.base_path / "xfmr_catalog_dry_type_distribution_transformers.csv")
        transformers = []

        for idx, row in reg_data_1.iterrows():
            ...
            # secondary_voltage = max(secondary_voltages)
            nt = max(secondary_voltages) / min(secondary_voltages)
            is_center_tapped = np.isclose(nt, 2.0)

            conn = ConnectionType.STAR if "Y/" in  row["secondary voltage"] else ConnectionType.DELTA
            ...
            xfmr = DistributionTransformerEquipment(
                name=row["catalog id"],
                pct_no_load_loss=row["no load loss - watts"] / (kva_rating * 1000) * 100.0,
                pct_full_load_loss=row["total loss - watts"] / (kva_rating * 1000) * 100.0,
